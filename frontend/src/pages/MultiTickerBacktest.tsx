...
import {
  getAvailableTickers, getSavedResults, runStockBacktest, runStockOptimization,
  type TickerInfo, type StockOptimizeResultEntry, type StockBacktestResponse,
  type StockBacktestParams, type StockOptimizeParams,
...
export default function MultiTickerBacktest() {
  const [view, setView] = useState<'results' | 'optimize' | 'backtest'>('results')
  ...
  const [optTf, setOptTf] = useState('5m')
  const [optIterations, setOptIterations] = useState(200)
  const [optMetric, setOptMetric] = useState('risk_adjusted')
  const [optQuantity, setOptQuantity] = useState(2)
  const [optimizing, setOptimizing] = useState(false)
  const [optResult, setOptResult] = useState<StockOptimizeResultEntry[] | null>(null)
  ...
  useEffect(() => {
    getAvailableTickers().then(setTickers).catch(() => {})
    getSavedResults()
  ...
  const runOptimize = () => {
    ...
    setOptError(null)
    setOptResult(null)
    const params: StockOptimizeParams = {
      ticker: optTicker,
      bar_interval: optTf,
      num_iterations: optIterations,
      target_metric: optMetric,
      top_n: 10,
      quantity: optQuantity,
    }
    runStockOptimization(params)
      .then((r) => setOptResult(r.results))
      .catch((err) => setOptError(err.response?.data?.detail || 'Optimization failed'))
  ...
  return (
    <div className="space-y-6">
      {/* Sub-nav */}
      <div className="flex bg-elevated rounded-lg p-0.5 w-fit">
        {(['results', 'optimize', 'backtest'] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => setView(tab)}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors capitalize ${
              view === tab ? 'bg-green-600 text-white' : 'text-secondary hover:text-primary'
            }`}
          >
            {tab === 'results' ? 'Top Setups' : tab === 'optimize' ? 'Optimize' : 'Drill-Down'}
          </button>
        ))}
      </div>
        ...
        <>
          {loadingResults ? (
            ...
            <div className="bg-surface rounded-lg p-8 text-center text-secondary">
              No saved results found. Run the optimizer first or use the Optimize tab.
            </div>
          ) : (
      ...
      {view === 'optimize' && (
        <>
          <div className="bg-surface rounded-lg p-6">
            <h2 className="text-lg font-semibold mb-4">Single-Ticker Options Optimization (0.40 delta)</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 text-sm">
              <label className="space-y-1">
                <span className="text-secondary">Ticker</span>
                <select value={optTicker} onChange={(e) => setOptTicker(e.target.value)}
                  className="w-full bg-elevated rounded px-2 py-1.5 text-heading">
                  {(tickers.length > 0 ? tickers.map((t) => t.ticker) : ['NVDA']).map((t) => (
              ...
              <label className="space-y-1">
                ...
                <select value={optTf} onChange={(e) => setOptTf(e.target.value)}
                  className="w-full bg-elevated rounded px-2 py-1.5 text-heading">
                  {['1m', '5m', '10m', '15m', '30m'].map((t) => (
                    <option key={t} value={t}>{t}</option>
                  ))}
                </select>
              </label>
              <label className="space-y-1">
                ...
                <input type="number" min={10} max={2000} step={50} value={optIterations}
                  onChange={(e) => setOptIterations(+e.target.value)}
                  className="w-full bg-elevated rounded px-2 py-1.5 text-heading" />
              </label>
              <label className="space-y-1">
              ...
              <label className="space-y-1">
                <span className="text-secondary">Contracts</span>
                <input type="number" min={1} max={100} value={optQuantity}
                  onChange={(e) => setOptQuantity(+e.target.value)}
                  className="w-full bg-elevated rounded px-2 py-1.5 text-heading" />
              </label>
              <div className="flex items-end">
                <button
                  onClick={runOptimize}
                  disabled={optimizing}
                  className="w-full px-4 py-1.5 rounded bg-green-600 hover:bg-green-500 text-white font-medium text-sm disabled:opacity-50"
                >
          ...
          {optimizing && <Spinner text={`Testing ${optIterations} combinations for ${optTicker} @ ${optTf}...`} />}

          {optResult && !optimizing && (
            <ResultsTable results={optResult} onDrillDown={drillDown} loading={btLoading} />
          )}
        </>
      )}
      ...
      {view === 'backtest' && (
        <>
          ...
          {!btResult && !btLoading && !btError && (
            <div className="bg-surface rounded-lg p-8 text-center text-secondary">
              Click "Drill Down" on any result to see the full backtest details.
            </div>
          )}
        </>
      )}
...
function ResultsTable({
  ...
  return (
    <div className="space-y-4">
      ...
      {tickerSummary.length > 3 && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
          {tickerSummary.map((r) => (
            <div key={r.ticker} className="bg-surface rounded-lg p-3 cursor-pointer hover:bg-hover"
      ...
      <div className="bg-surface rounded-lg p-6 overflow-x-auto">
        <h2 className="text-lg font-semibold mb-4">All Results ({results.length})</h2>
        <table className="w-full text-sm">
          ...
          <tbody>
            {sorted.map((r, i) => (
              <tr key={`${r.ticker}-${r.timeframe}-${i}`} className="border-b border-row hover:bg-hover">
                <td className="py-2.5 pr-3 text-muted">{i + 1}</td>
                <td className="py-2.5 pr-3 font-semibold">{r.ticker}</td>
...
// ── Backtest result components (same patterns as Backtest.tsx) ────
